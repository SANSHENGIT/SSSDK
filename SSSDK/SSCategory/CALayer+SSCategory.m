...
@implementation CALayer (SSCategory)
...
- (UIViewContentMode)contentMode {
    return __CAGravityToUIViewContentMode(self.contentsGravity);
}

- (void)setContentMode:(UIViewContentMode)contentMode {
    self.contentsGravity = __UIViewContentModeToCAGravity(contentMode);
}


#pragma makr - Animation
- (void)setFadeAnimationWithDuration:(NSTimeInterval)duration curve:(UIViewAnimationCurve)curve {
    ...
    [self addAnimation:transition forKey:@"__Category.fade"];
}

- (void)removePreviousFadeAnimation {
    [self removeAnimationForKey:@"__Category.fade"];
}


- (CAAnimation *)animationShake:(NSArray *)rotations
...
- (CATransition *)transitionWithAnimType:(CGAffineTransformType)animType
    ...
    transition.type     = [self privateAnimaTypeWithTransitionType:animType];//动画类型
    transition.subtype  = [self privateAnimaSubtype:subType];//动画方向
    transition.timingFunction = [CAMediaTimingFunction functionWithName:[self privateCurve:curve]];//缓动函数
...
/**  返回动画曲线 */
- (NSString *)privateCurve:(CGAffineTransformCurve)curve{
    ...
    return [self privateObjFromArray:funcNames index:curve isRamdom:(CGAffineTransformCurveRamdom == curve)];
}

/**  返回动画方向 */
- (NSString *)privateAnimaSubtype:(CGAffineTransformSubType)subType{
    //设置转场动画的方向
    NSArray *subtypes = @[kCATransitionFromTop,kCATransitionFromLeft,kCATransitionFromBottom,kCATransitionFromRight];
    
    return [self privateObjFromArray:subtypes index:subType isRamdom:(CGAffineTransformSubTypeFromRamdom == subType)];
}

/** 返回动画类型 */
- (NSString *)privateAnimaTypeWithTransitionType:(CGAffineTransformType)type{
    
    //设置转场动画的类型
    NSArray *animArray = @[@"rippleEffect",@"suckEffect",@"pageCurl",@"oglFlip",@"cube",@"reveal",@"pageUnCurl"];
    
    return [self privateObjFromArray:animArray index:type isRamdom:(CGAffineTransformTypeRamdom == type)];
}

/** 统一从数据返回对象 */
- (id)privateObjFromArray:(NSArray *)array index:(NSUInteger)index isRamdom:(BOOL)isRamdom{
