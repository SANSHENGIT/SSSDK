...
#import "SSAdScrollView.h"

typedef void(^SSAdClickHandler)(NSInteger index);

@interface SSAdImageView : UIView
@property (strong, nonatomic) UIImageView *imageView;
@property (strong, nonatomic) UIControl *control;
@property (assign, nonatomic) NSInteger actionTag;
@property (copy, nonatomic) NSString *imageUrl;
@property (strong, nonatomic) UIImage *placeholder;
@property (copy, nonatomic) SSAdClickHandler handler;
...
- (instancetype)initWithFrame:(CGRect)frame imageURL:(NSString*)url placeholderImage:(UIImage*)placeholder {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.imageView setImageWithURL:url defineImage:placeholder];
...
- (void)setImageUrl:(NSString *)imageUrl {
    [self.imageView setImageWithURL:imageUrl defineImage:self.placeholder];
}
...
@interface SSAdScrollView () <UIScrollViewDelegate>
...
@property (strong, nonatomic) SSAdImageView *currentImage;
@property (strong, nonatomic) SSAdImageView *preImage;
@property (strong, nonatomic) SSAdImageView *nextImage;
//上一次scrollView的偏移量
...
- (instancetype)initWithFrame:(CGRect)frame withContents:(NSArray<NSString*>*)contents andScrollDelay:(CGFloat)delay {
    SSAdScrollView *view = [[SSAdScrollView alloc] initWithFrame:frame withScrollDelay:delay];
...
- (void)setupScrollView {
    ...
    self.pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(0, self.height-SSAdPageControlHeight, kWidth, SSAdPageControlHeight)];
...
- (void)initContents {
    [self initData];
    [self clearViewsTagAbove:SSAdImageViewTagBase];
...
- (void)setupImageViews {
    ...
    for (NSInteger i = 0; i < self.imageCount; i++) {
        @autoreleasepool {
            SSAdImageView *imageView = [[SSAdImageView alloc]initWithFrame:CGRectMake(self.width*i, 0, self.width, self.height) imageURL:_mImageArray[i] placeholderImage:self.placeholderImage];
            imageView.tag = i+SSAdImageViewTagBase;
...
- (void)autoScrollAd {
    ...
    [self resetThreeImages];
    //滚动到下一页
    [UIView animateWithDuration:SSAdAutoScrollDuration animations:^{
        @__StrongObj(self);
        self.scrollView.contentOffset = CGPointMake((x+1)*self.width, 0);
        if (self.animationType == SSAdImageAnimationFadeInOut) {
            //自动滚动只需要考虑当前页和下一页
            self.currentImage.hideCover = YES;
            self.nextImage.hideCover = NO;
        } else if (self.animationType == SSAdImageAnimationRotation) {
            //自动滚动只需要考虑向左，逆时针方向
            self.currentImage.transform = CGAffineTransformMakeRotation(-M_PI_4);
        } else if (self.animationType == SSAdImageAnimationScale) {
...
- (void)resetThreeImages {
    ...
    if (self.imageViewArray.count > 2) {
        ...
        } else if (self.animationType == SSAdImageAnimationDown ||
                   self.animationType == SSAdImageAnimationUp) {
        ...
        } else if (self.animationType == SSAdImageAnimationBlur) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    switch (self.animationType) {
        ...
        case SSAdImageAnimationDown:
            self.currentImage.frame = CGRectMake(self.currentImage.frame.origin.x, ratio * self.height, CGRectGetWidth(self.currentImage.frame), CGRectGetHeight(self.currentImage.frame));
            break;
            
        case SSAdImageAnimationBlur:
...
#pragma mark setter
- (void)setAnimationType:(SSAdImageAnimation)animationType {
    _animationType = animationType;
    [self resetThreeImages];
}

- (void)setDelay:(CGFloat)delay {
    _delay = delay > 0 ? delay : SSAdDefaultDelay;
}
