...
#import "SSHTTPRequestManager.h"
#import <objc/runtime.h>

@implementation SSHTTPRequestManager
...
+ (NSURLSessionTask *)GET:(NSString *)URL
    ...
    if (responseCache) {
        responseCache([SSHTTPRequestCache getResponseCacheForKey:cacheKey]);
    }
    AFHTTPSessionManager *manager = [self manager];
    return [manager GET:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (cache) {
            [SSHTTPRequestCache saveResponseCache:responseObject forKey:cacheKey];
        }
        if (success) {
...
+ (SSHTTPRequestCenter *)manager {
    static SSHTTPRequestCenter *manager = NULL;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [SSHTTPRequestCenter sharedCenter];
